...
def printIntro(fileName): #Funciona
	...
	'''
	file = open('/home/camilo/workspace/Laboratorio6/'+fileName+'.txt')
	print(file.read())
	file.close()
...
def getRandomSudoku(n_sud): #Funciona
	...
	'''
	n = randint(1,n_sud)
	file = open('/home/camilo/workspace/Laboratorio6/sd'+str(n)+'.txt')
	M = list(file)
	for i in range(9):
...
def Clear(command, M, C, D): # Funciona
	'''
	...
	try:
		if (command[6] in Letras) and (int(command[7]) in range(1,10)): # Puede presentar error si el string command[7] no es un número: ValueError en int(command[7])
			if C[Letras.index(command[6])][int(command[7])-1] == '0': # Búsqueda de pista en Copy
				D[command[6:]] = M[Letras.index(command[6])][int(command[7])-1] # Almacenar coordenada y número para la función Undo.
				M[Letras.index(command[6])][int(command[7])-1] = '0'
				return M, D
			else: # Búsqueda de pista en Copy
...
def Undo(Historial, M, C, D):
	'''Toma el historial de cambios y deshace el último de ellos.
		Los cambios son de tipo 'c4 2' y 'clear b3'.'''
	if len(Historial[-1])==4:
		for i in reversed(range(len(Historial)-1)):
			if Historial[i][:2] == Historial[-1][:2]:
				M[Letras.index(Historial[-1][0])][int(Historial[-1][1])-1] = Historial[i][3]
				break
		else:
			M[Letras.index(Historial[-1][0])][int(Historial[-1][1])-1] = '0'
	elif len(Historial[-1])==8: # Revisar
		M[Letras.index(Historial[-1][-2])][int(Historial[-1][-1])-1] = D[Historial[-1][6:]]
	del Historial[-1]
	return M, Historial